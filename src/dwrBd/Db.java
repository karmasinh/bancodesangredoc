...
public class Db {
	...
	// richard
	private String Password = "whitecat01";
	...
	public Db() {
		...
		try {
			this.con = DriverManager.getConnection("jdbc:postgresql://" + this.getHost() + "/" + this.getBase(),
	 ...
	 * this.stmt = this.con.createStatement();
	 * this.rs =
	 * stmt.executeQuery("select max(*) as valor from  "+tabla+" where "+campo+"="+
	 * campob);
	 * this.rs.next();
	 * valor = rs.getString("valor");
	 * if (valor==null){
