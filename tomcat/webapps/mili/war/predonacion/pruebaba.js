...
if (kk=0) {
sval=1;
 DWRUtil.setValue("mili","<input type='submit' value='Agregar' onclick='ejecutarInsertar();'>");
}
 }*/
/////*****fin validaciones*********************
...
var fecha="12-12-12";
ListaPreDAO.conteo(validar,id_prueba,valores,id_aten,cod_do);
}
...
ListaPreDAO.InsertarC(Find,id_prueba,valores,id_aten,cod_do,fecha_prueba,hora_prueba,cod_per,id_horario);
var id_aten=DWRUtil.getValue("id_aten");
ListaPreDAO.FinalizarTodo(qq,id_aten);
}
function qq()
{
...
ListaPreDAO.InsertarC(nada,id_prueba,valores,id_aten,cod_do,fecha_prueba,hora_prueba,cod_per,id_horario);
var id_aten=DWRUtil.getValue("id_aten");
...
//ListaPreDAO.InsertarC(nada,id_prueba,valores,id_aten,cod_do);
ListaPreDAO.InsertarC(nada,id_prueba,valores,id_aten,cod_do,fecha_prueba,hora_prueba,cod_per,id_horario);
*/
ListaPreDAO.FinalizarTodo(listar1,id_aten);
}
...
$(lista1).style.display='none';
ListaPreDAO.mostrarLista(mostrarListaDonantes);
ListaPreDAO.cont1(mostrarconteocombos);
...
ListaPreDAO.mostrarLista3(mostrarListaDonantes1);
fechas();
horas();
}
...
function listarp(id_prueba){
 ListaPreDAO.mostrarLista4(nivel1,id_prueba);
}
...
if(gel[i]=="1"){
ListaPreDAO.InsertarDonante(Resultado,valores,cod_do);
}
    if(gel[i]!="EmCi"){
	ListaPreDAO.InsertarC(Resultado,gel[i],valores,id_aten,cod_do,fecha_prueba,hora_prueba,cod_per,id_horario);
	ListaPreDAO.conteo(validar,i,valores,id_aten,cod_do);
    }	
}
ListaPreDAO.cont2(mostrarCantidad,cod_do,id_aten);
}
...
var cod_per=DWRUtil.getValue("cod_per");
var id_horario=DWRUtil.getValue("id_horario");
//ListaPreDAO.InsertarC(Resultado,id_prueba,valores,id_aten,cod_do,cod_per,id_horario);
ListaPreDAO.InsertarC(Resultado,id_prueba,valores,id_aten,cod_do,fecha_prueba,hora_prueba,cod_per,id_horario);
}
