...
public class ListaSereDAO {

public Collection mostrarLista() {

	Collection lista = new Vector();
	String  sqla ="select a.id_aten,d.cod_do,p.nom,p.apepa,p.apema,rp.resultado  from atencion a,persona p,donante d,evaluacion e,resultado_prueba rp where(a.id_aten=e.id_eva)and(d.cod_do=a.cod_do)and(p.id_per=d.id_per)and ((a.atendidop='A')and(a.atendidoh='A')and(a.atendidos='T')) and((a.id_aten=rp.id_aten)and(rp.resultado='Donante  Apto')and(instancia='inmunohematologia'));";
...
public Collection mostrarLista3() {
	...
	Collection lista = new Vector();
	String  sqla ="select p.id_prueba,p.nombre_tp,p.tipo_componente  from prueba p  where (tipo='inmunoserelogia')and(positivo='N') order by(id_prueba)  ;";
...
public Collection mostrarLista4(int id_prueba) {
	...
	Collection lista = new Vector();
	String  sqla ="select  v.valores,v.id_prueba,v.id_valor from prueba p,valores v where (p.tipo='inmunoserelogia')and(p.id_prueba = v.id_prueba)and(p.id_prueba="+id_prueba+")and(v.positivo='N')  ;";
	        Connection ca = null;
		Statement stmta = null;
		try {
			ca = DBUtils.openConnection();
...
public int cont() {
	...
	String  sql ="select count(*) as cont  from prueba where(tipo='inmunoserelogia') ;";
...
public void InsertarC(int id_prueba,String valores,int id_aten,String cod_do,String hora_prueba,String fecha_prueba,String cod_per,int id_horario)
{
                String sqlpw="";
		String m="miliydavid";
	        String instancia="inmunoserelogia";
...
public void  ModificarC(int id_prueba,String valores,int id_aten,String cod_do,String hora_prueba,String fecha_prueba,String cod_per,int id_horario)  
{
		...
		//sqlpw = "update resultado_prueba  set resultado = '"+valores+"',hora_prueba='"+hora_prueba+"',fecha_prueba='"+fecha_prueba+"',cod_per='"+cod_per+"',id_horario="+id_horario+"  where (id_prueba = "+id_prueba+")and(id_aten="+id_aten+")and(cod_do='"+cod_do+"') ;";
...
public void FinalizarTodo(int id_aten)  
{
	        ...
	        sqlpw = "UPDATE atencion SET atendidos = '"+atendidos+"' , atendido='"+atendido+"' WHERE id_aten = "+id_aten+"  ;";
...
public int cont1() {
	...
	String  sql ="SELECT COUNT(*) AS CONT1 FROM PRUEBA WHERE(TIPO='inmunoserelogia')AND(TIPO_COMPONENTE='2') ;";
...
public int contd() {
	...
	String  sql ="select count(*) as cont  from prueba where(tipo='inmunoserelogia')and(positivo='N') ;";
...
//	String sql = "select r.resultado,r.id_prueba,p.tipo_componente,r.id_aten,r.cod_do  from resultado_prueba r,prueba p,atencion a where (r.cod_do='"+cod_do+"')and(p.tipo='inmunohematologia')and(a.cod_do=r.cod_do)and(r.id_prueba=p.id_prueba)and(a.id_aten=(select max(id_aten) from atencion))and(a.id_aten=r.id_aten) ORDER BY id_prueba ";
	String sql = "select r.resultado,r.id_prueba,p.tipo_componente,r.id_aten,r.cod_do from resultado_prueba r,prueba p, atencion a where(r.cod_do='"+cod_do+"')and(a.id_aten=(select max(t.id_aten) from resultado_prueba t where(t.cod_do='"+cod_do+"') and(t.instancia='inmunoserelogia') ))and(r.id_prueba=p.id_prueba)and(r.instancia='inmunoserelogia') order by(id_prueba);";
	        Connection ca = null;
		Statement stmta = null;
		try {
			ca = DBUtils.openConnection();
