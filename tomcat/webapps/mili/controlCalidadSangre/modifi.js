...
ListaControlDAO.contd(mostrarpruebas);
var cod_do = DWRUtil.getValue("cod_do"); 
var fecha_a = DWRUtil.getValue("fecha_a"); 
ListaControlDAO.mostrarResultado_prueba(nivel1,cod_do);
...
var valores=DWRUtil.getValue(""+gel[i]+"");
	if(i==v){
	    if(gel[i]!=8){ ListaPreDAO.ModificarC(Resultado,gel[i],valores,id_aten,cod_do,fecha_prueba,hora_prueba,cod_per,id_horario); }
	    if(a1>0){
	    valores="Donante no Apto";
	    DWRUtil.setValue("8",valores);     
	    ListaPreDAO.ModificarC(Resultado,"8",valores,id_aten,cod_do,fecha_prueba,hora_prueba,cod_per,id_horario);}
	    else{
